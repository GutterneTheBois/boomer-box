...
export class PlayCommand extends Command {
	...
	public override async chatInputRun(
		...
		if (!results.hasTracks())
			return interaction.reply({
				content: `${emojis.error} | I don't understand, speak clearly!`,
				ephemeral: true,
			});
		...
		try {
			...
			return interaction.editReply({
				content: `${emojis.success} | Successfully found${
					res.track.playlist
						? ` my records from: **${res.track.playlist.title}**`
						: `: **${res.track.title}**`
				}`,
			});
		} catch (error: any) {
			await interaction.editReply({
				content: `${emojis.error} | I can't find my glasses!`,
			});
